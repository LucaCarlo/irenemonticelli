...
const htlCards = TL.map(([y, t]) => `
        <article class="qs-tl2-item">
          <div class="qs-tl2-top"><span class="qs-tl2-dot" aria-hidden="true"></span></div>
...
<style>
  ...
  /* Timeline pulita: linea in alto + riquadri sotto (niente clipping sopra/sotto) */
  .qs-tl2{position:relative;margin:56px 0 0;left:50%;width:100vw;transform:translateX(-50%)}
  .qs-tl2-viewport{overflow:hidden;position:relative;cursor:none;padding:6px 0 14px}
  .qs-tl2-track{display:flex;padding:0 5vw;position:relative;transition:transform .8s cubic-bezier(.7,0,.2,1);will-change:transform}
  .qs-tl2-track::before{content:"";position:absolute;left:5vw;right:5vw;top:19px;height:2px;background:linear-gradient(90deg,rgba(0,0,0,.05),var(--yellow,#e0aa00) 12%,var(--yellow,#e0aa00) 88%,rgba(0,0,0,.05))}
  .qs-tl2-item{flex:0 0 440px;width:440px;display:flex;flex-direction:column;padding:0 22px}
  .qs-tl2-top{height:40px;display:flex;align-items:center;position:relative;z-index:1}
  .qs-tl2-dot{width:18px;height:18px;border-radius:50%;background:var(--yellow,#e0aa00);box-shadow:0 0 0 6px rgba(224,170,0,.16)}
  .qs-tl2-card{position:relative;margin-top:24px;background:#fff;border:1px solid rgba(0,0,0,.06);border-radius:18px;padding:30px 32px;box-shadow:0 26px 56px -30px rgba(0,0,0,.3)}
  .qs-tl2-card::before{content:"";position:absolute;left:9px;top:-24px;width:2px;height:24px;background:rgba(224,170,0,.5)}
  ...
  .qs-cursor{position:fixed;left:0;top:0;width:64px;height:64px;border-radius:50%;background:var(--yellow,#e0aa00);color:#fff;display:flex;align-items:center;justify-content:center;font-size:28px;pointer-events:none;z-index:90;transform:translate(-50%,-50%) scale(0);transition:transform .18s cubic-bezier(.7,0,.2,1);box-shadow:0 10px 28px -10px rgba(224,170,0,.6)}
  .qs-cursor.on{transform:translate(-50%,-50%) scale(1)}
  .qs-tl2-hint{text-align:center;font-size:12px;letter-spacing:.18em;text-transform:uppercase;color:#9a8f7a;margin:22px 0 0}
  @media(max-width:760px){.qs-tl2-track{padding:0 16px}.qs-tl2-track::before{left:16px;right:16px}.qs-tl2-item{flex-basis:84vw;width:84vw;padding:0 12px}.qs-tl2-year{font-size:32px}.qs-cursor{display:none}.qs-tl2-viewport{cursor:default}}
